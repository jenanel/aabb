...
if(!exists("Xylocopaharmonthreattmodeltable", mode = "function")) source(here::here ("functions",
  "Xylocopaharmonthreattmodeltable.r"))



if(!exists("pilotanalysistable", mode = "function")) source(here::here("functions", 
                                                                    "xylocopapilotanalysistable.r"))
...
#NOTE!!! THIS NEEDS TO BE REPAIRED! I NEED TO MERGE IN THE FGENUS COLUMN TO THE BEE TABLE THEN USE THAT TO MERGE INTO THE XYLOCLEANEDDATASET! BUT I WONT DO THT BC I DID IT RIGHT IN THE ALL SPECIES LOOP SO THIS IS NOT WORTHWHILE! 

CarpenterBeeTable <- 
  BeeTable %>%
  filter(grepl("Xylocopa", BSpecies)) %>% #1028 rows 
  merge(FlowerTableAllDupesPresent, by = c("SurveyNum", "FNum"), all.x = TRUE) %>%
  mutate(BSpecies = "Xylocopa species") %>%
  select(!c("IDnew","CountofLots":"corrFloralRank", "Sex","FNum")) %>%
...
xylocleanedgenusleveldataset <- #returns 11542 rows, math predicts 11542! 
  genusleveldatafileinprogressunfiltered %>%
  full_join(genustofamilytable, by = "FGenus") %>%
  merge(CarpenterBeeTable, by = c("SurveyNum", "FGenus"), all.x = TRUE) %>%
  replace(is.na(.), 0) %>%
  left_join(rawsurveyconditionstable, by = "SurveyNum") %>%
  select(!BSpecies) %>%
  group_by(SurveyNum) %>%
  rename(CBcount = Count) %>%
  mutate(SumCBcount = sum(CBcount)) %>%
  filter(SumCBcount > 0) %>%
  mutate(CorrCBcount = CBcount / SumCBcount) %>%
  mutate(CorrRankAvgFlRankXCorrCBcount = CorrRankAvgFlRank * CorrCBcount) %>%
  mutate(SeasonalChunk = case_when(
  ...
  select(ID:ErrorCode,
         ...
         CBcount:CorrRankAvgFlRankXCorrCBcount
          ) %>%
  filter(!grepl("NoFRHASBEES", ErrorCode)) %>% # removes surveys with that error code
  add_count(FGenus, name = "FlowerSampleSize")%>%
  group_by(SurveyNum) %>%
  mutate("SumofBBcount" = sum(BBcount)) %>% #to verify that the filter worked
  mutate("NumberofFlowers" = n()) %>% #to verify that the filter worked
  filter(NumberofFlowers > 4) %>% #survey 88 should no longer be present
  ungroup()
...
finalharmonthreattmodeltable<- xylocopaharmonthreattmodeltable(xylocleanedgenusleveldataset)
...
JenanAndSamPilotAnalysisTable <- xylocopapilotanalysistable(xylocleanedgenusleveldataset)
...
entiregenuslevelanalysistable <-
  xylocleanedgenusleveldataset %>%
  dplyr::summarise(
    .by = FGenus,
    NumberSurveys = n(),
    #NumberSurveysWithVisitation = sum(CBcount >0),
    ProportionSurveysWithVisitation = sum(CBcount > 0) / n(),
    ...
    SumCBCount = sum(CBcount),
    AvgCBCount = mean(CBcount),
    AvgCorrCBCount = mean(CorrCBcount),
    CBScore.CorrRankAvgFlRankXCorrCBCount = mean(CorrRankAvgFlRankXCorrCBcount),
    StandardDeviationCBScore = sd(CorrRankAvgFlRankXCorrCBcount, na.rm = TRUE),
    StandardErrorCBScore = (sd(CorrRankAvgFlRankXCorrCBcount, na.rm = TRUE)) /
      sqrt(NumberSurveys)) %>%
  dplyr::mutate(across(where(is.numeric),
                       ~ round(., 2))) %>% # round values to make them presentable)
  dplyr::arrange(desc(CBScore.CorrRankAvgFlRankXCorrCBCount)) %>%
  dplyr::left_join(., finalharmonthreattmodeltable, by = "FGenus") %>%
  dplyr::select(c(FGenus:StandardErrorCBScore, vRank)) %>% #remove all the other Harmon-Threatt columns except vRank)
...
combined.summary.tables.list <-
  list(
    "xylocopaentiredataset" = entiregenuslevelanalysistable
  )

writexl::write_xlsx(combined.summary.tables.list,
                   here("data",
                       "summarized.data", 
                       "xylocopa.summarytables.genuslevel.universalfiltersapplied.xlsx")) 
